...
final class AgileExportModelTest extends TestCase
{
    public function testMonetaryValueVoMappingRoundTrip(): void
    {
        $model = new AgileExport();

        // Seed raw attributes to simulate DB
        $model->setRawAttributes([
            'value_exc_vat' => 0.0550,
            'value_inc_vat' => 0.0660,
        ], true);

        // Accessors read via VO
        $this->assertEqualsWithDelta(0.0550, $model->value_exc_vat, 1e-6);
        $this->assertEqualsWithDelta(0.0660, $model->value_inc_vat, 1e-6);

        // Mutate using accessors; ensure VO updates and attributes reflect
        $model->value_exc_vat = 0.0700;
        $model->value_inc_vat = 0.0840;

        $this->assertEqualsWithDelta(0.0700, $model->value_exc_vat, 1e-6);
        $this->assertEqualsWithDelta(0.0840, $model->value_inc_vat, 1e-6);
    }
    ...
    public function testTimeIntervalVoMappingRoundTrip(): void
    {
        ...
        $from = CarbonImmutable::parse('2025-09-28 11:00:00', 'UTC');
        $to   = CarbonImmutable::parse('2025-09-28 11:30:00', 'UTC');
