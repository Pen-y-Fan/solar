...
final class AgileChartInteractivityPluginsTest extends TestCase
{
    ...
    public function testItIncludesZoomAndPanConfig(): void
    {
        ...
        // Slower, more controlled zoom on mouse wheel
        $this->assertArrayHasKey('speed', $zoom['zoom']['wheel']);
        $this->assertIsNumeric($zoom['zoom']['wheel']['speed']);
        $this->assertLessThanOrEqual(0.1, $zoom['zoom']['wheel']['speed']);
        ...
        // Require a small movement before panning to avoid accidental drags
        $this->assertArrayHasKey('threshold', $zoom['pan']);
        $this->assertIsInt($zoom['pan']['threshold']);
        $this->assertGreaterThanOrEqual(10, $zoom['pan']['threshold']);
    }
}
