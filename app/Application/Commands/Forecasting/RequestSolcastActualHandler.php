...
final class RequestSolcastActualHandler implements CommandHandler
{
    ...
    public function handle(Command $command): ActionResult
    {
        ...
        try {
            ...
            Log::warning('Unhandled exception during actual forecast action', ['exception' => $e->getMessage()]);
            $this->allowance->recordFailure($endpoint, 500);
            return ActionResult::failure('Unexpected error during Solcast actual forecast', 'external_failure');
        }
    }
}
